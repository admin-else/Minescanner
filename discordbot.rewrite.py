import interactions, sqlite3, datetime, os, dotenv
...
def createServerEmbed(server, index, lastindex, searchoptions):
    ...
    ip, serverdescription, isColored, serverversion, ismodded, maxplayers = server[1], server[4], server[5], server[6], server[7], server[3]
...
async def server(ctx: interactions.CommandContext,
    ...
    global datalist
    datalist[ctx.channel_id] = {'servers':list(c.fetchall()),'page':0,'searchoptions':searchoptions}
    datalist = datalist[ctx.channel_id]

    c.close()

    if len(datalist['servers'])==0:
        await ctx.send('the search u did did not yield any results ):')
        return

    await ctx.send('', embeds=createServerEmbed(datalist['servers'][datalist['page']], datalist['page']+1, len(datalist['servers']), datalist['searchoptions']), components=getComponents())

@bot.component('previousserver')
async def button_response(ctx):
    global datalist
    if ctx.channel_id not in datalist:
        await ctx.edit('I dont have any server selected do /server.')
        return
    if serverindex>=len(datalist[ctx.channel_id]['servers']):
        serverindex-=1
        await ctx.edit('')
        return
...
async def button_response(ctx):
    ...
    if ctx.channel_id not in datalist:
        await ctx.edit('I dont have any server selected do /server.')
        return
    datalist[ctx.channel_id]['page']+=1
    if serverindex>=len(datalist[ctx.channel_id]['servers']):
        datalist[ctx.channel_id]['page']-=1
        await ctx.edit('')
        return
    await ctx.edit('', embeds=createServerEmbed(datalist['servers'][datalist[ctx.channel_id]['page']], datalist[ctx.channel_id]['page']+1, len(datalist)), components=getComponents())
